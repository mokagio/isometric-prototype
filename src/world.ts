// Tile identity is its [column, row] in the sheet. Palette drawn from the
// grass column (1) plus a water tile for the pond.
...
export function generateWorld(cols: number, rows: number, seed = 1337): World {
  ...
  const at = (col: number, row: number): Tile => {
    const dx = col - pondX;
    const dy = row - pondY;
    const wobble = pondR * (0.75 + 0.5 * noise(col, row, seed + 9));
    if (dx * dx + dy * dy < wobble * wobble) return WATER;
