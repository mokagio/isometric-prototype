export const ATTACK_DURATION = 0.36;
// The blade connects partway in rather than on the last frame. An enemy covers
// the gap from melee range to contact in ~0.3s, so a blow that waits for the
// animation to finish always arrives after the hit it was meant to prevent.
export const ATTACK_HIT_AT = 0.15;
...
export class Swing {
  ...
  /** Seconds into the current swing, for picking the blade angle. */
  get time(): number {
    return this.t ?? 0;
  }

  /** 0 at the wind-up, 1 at the follow-through. */
  get progress(): number {
    return Math.min(1, (this.t ?? 0) / ATTACK_DURATION);
  }
