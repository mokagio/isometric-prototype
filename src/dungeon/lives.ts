...
// An enemy that reaches the hero parks there and keeps swinging, so without a
// window of immunity one skeleton empties the bar in a handful of frames.
export const INVULN = 1; // seconds of immunity after a hit
export const DEATH_FADE = 1.4; // seconds the hero takes to fade out
...
export class Lives {
  ...
  /** Seconds since the hero went down. */
  ...
  update(dt: number): void {
    if (this.alive) this.invulnT = Math.max(0, this.invulnT - dt);
    else this.deathT += dt;
  }
